...
try {
  ...
  try {
    for (const locale of ['fa', 'en']) {
      for (const mode of modes) {
        ...
        await page.screenshot({
          path: `docs/evidence/TASK-0301-article-${locale}-${mode.name}.png`,
          fullPage: true,
          animations: 'disabled',
        });
        await page.close();
    ...
    await printPage.goto(`${baseUrl}/en/articles/document-aware-development/`);
    await printPage.emulateMedia({ media: 'print' });
    await printPage.screenshot({
      path: 'docs/evidence/TASK-0301-article-en-print.png',
      fullPage: true,
      animations: 'disabled',
    });
