...
const slug = 'project-should-explain-itself';
const sequences = [
  ['ADR', 'SPEC', 'TASK', 'CODE'],
  [
  ...
  ['Documentation', 'Implementation'],
  ['Governance', 'Decisions', 'Specifications', 'Tasks', 'Implementation'],
];

for (const locale of ['fa', 'en']) {
  test(`DaD II ${locale}: static diagrams, translation, metadata and accessibility`, async ({
    browser,
    baseURL,
    request,
  }) => {
    ...
    await expect(page.locator('.content-diagram')).toHaveCount(5);
    ...
    await expect(page.locator('.article-prose a').first()).toHaveAttribute(
      'href',
      `/${locale}/articles/building-easier-than-understanding/`,
    );
    await expect(
      page.locator('.content-diagram--reciprocal .content-diagram__connector'),
    ).toHaveText('↔');
    await expect(
      page
        .locator('.content-diagram')
        .nth(3)
        .locator('.content-diagram__connector'),
    ).toHaveText('→');
    ...
    const searchUrls = await accessiblePage.evaluate(
      ...
      locale === 'fa' ? 'Governance' : 'Governance',
    );
