...
test('published Persian article metadata is indexable and truthful', async ({ page }) => {
...
test('robots and sitemap expose the production site to crawlers', async ({ page }) => {
...
test('RSS feeds reflect currently published articles only', async ({ page }) => {
  ...
  expect(faText).not.toMatch(/document-aware-development|signals-before-solutions/);
  expect(enText).not.toMatch(/document-aware-development|signals-before-solutions/);
});
